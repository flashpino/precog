...
<div class="flex items-center justify-between mb-4">
    <h3 class="text-xs font-bold text-industrial-gray-900 uppercase tracking-widest flex items-center gap-2">
        <span class="material-symbols-outlined text-primary text-sm">edit</span> Editar Equipamento: {{ $sensor->device_id }}
    </h3>
    ...
    <form action="{{ route('sensors.update', $sensor) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="client_id" id="client_id" required class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" data-token="{{ $client->influx_token }}" {{ old('client_id', $sensor->client_id) == $client->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="device_id" id="device_id" value="{{ old('device_id', $sensor->device_id) }}" required class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary font-mono text-sm uppercase" placeholder="Ex: PRECOG_ESTOQUE_01">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="label" id="label" value="{{ old('label', $sensor->label) }}" class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="location" id="location" value="{{ old('location', $sensor->location) }}" class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                </div>

                <div class="flex items-center gap-2 pt-2">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $sensor->is_active ? 'checked' : '' }} class="rounded border-industrial-gray-300 text-primary focus:ring-primary">
                    <label for="is_active" class="text-sm font-bold text-industrial-gray-700">Equipamento Ativo no Monitoramento</label>
                </div>
            </div>
            ...
            <div class="space-y-4 bg-industrial-gray-50 p-4 rounded border border-industrial-gray-200">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.1" name="temp_min" id="temp_min" value="{{ old('temp_min', $sensor->temp_min) }}" class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.1" name="temp_max" id="temp_max" value="{{ old('temp_max', $sensor->temp_max) }}" class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 pt-2">
                    <div>
                        ...
                        <input type="number" step="0.1" name="hum_min" id="hum_min" value="{{ old('hum_min', $sensor->hum_min) }}" class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.1" name="hum_max" id="hum_max" value="{{ old('hum_max', $sensor->hum_max) }}" class="w-full border-industrial-gray-300 rounded focus:border-primary focus:ring-primary text-sm">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-industrial-gray-100 flex justify-end">
            <button type="submit" class="bg-primary hover:bg-blue-700 text-white text-xs font-bold uppercase tracking-widest py-2 px-6 rounded transition-colors flex items-center gap-2 shadow-sm">
                <span class="material-symbols-outlined text-sm">save</span> Atualizar Equipamento
            </button>
        </div>
    </form>
</div>
