...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'create') {
            ...
            if ($client_id > 0 && !empty($name) && !empty($phone)) {
                Database::execute(
                ...
                // Disparar webhook de novo contato
                $client = Database::queryOne("SELECT name, company FROM clients WHERE id = ?", [$client_id]);
                $payload = [
                    'event'    => 'contact_added',
                    'contact'  => [
                        'name'  => $name,
        ...
        if ($action === 'update') {
            ...
            if ($id > 0 && $client_id > 0 && !empty($name)) {
                Database::execute(
                    ...
                    [$client_id, $name, $phone, $active, $id]
                );
...
// Filtro por cliente
$filterClient = intval(isset($_GET['client_id']) ? $_GET['client_id'] : 0);
